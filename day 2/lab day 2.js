...
.	Try `updateMany`
.  Try using the `replaceOne` operation.*/
//updateOne
...
db.employees.updateOne({ name: "Hassan" }, { $setOnInsert: { salary: 5000, department: "HR" } }, { upsert: true });
db.employees.updateOne({ name: "Hassan" }, { $max: { salary: 6000 } });
db.employee.updateOne({ name: "Alaa" }, { $setOnInsert: { overtime: 10 } }, { upsert: true });
db.employee.updateOne({ name: "Alaa" }, { $min: { overtime: 5 } });
db.employee.updateOne({ name: "Alaa" }, { $inc: { age: 1 } });
db.inventory.updateOne({ item: "notebook" }, { $mul: { quantity: 2, price: 1.5 } });
